...
! function(a, b) {
	...
	var c, d, e = {
			getPath: function() {
				...
				//最后一个script标签不一定是layer.js的引用标签所以重写获取layer引用路径的方法
				var a = document.scripts,
					b, c;
				for(var i = 0; i < a.length; i++) {
					if(a[i].src.indexOf("layer.js") != -1) {
						b = a[i];
						c = b.src;
					}
				}
	...
	h.anim = ["layui-anim", "layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06"], g.pt.config = {
		...
		return f.zIndex = g, b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (g - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + ('0.3' || f.shade[0] || f.shade) + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>' : "", '<div class="' + h[0] + " " + (h.anim[f.shift] || "") + (" layui-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "" : " layui-layer-border") + " " + (f.skin || "") + '" id="' + h[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object" : "string") + '" style="z-index: ' + g + "; width:" + f.area[0] + ";height:" + f.area[1] + (f.fix ? "" : ";position:absolute;") + '">' + (a && 2 != f.type ? "" : k) + '<div id="' + (f.id || "") + '" class="layui-layer-content' + (0 == f.type && -1 !== f.icon ? " layui-layer-padding" : "") + (3 == f.type ? " layui-layer-loading" + f.icon : "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="layui-layer-ico layui-layer-ico' + f.icon + '"></i>' : "") + (1 == f.type && a ? "" : f.content || "") + '</div><span class="layui-layer-setwin">' + function() {
		...
		if(!c("#" + b.id)[0]) {
			...
			a.vessel(j, function(d, e) {
				if(top != window && d[0]) {
					$(top.document.body).find('.layer-shade').show();
				}
		...
		if(b[0]) {
			...
			if($('.layui-layer-shade').length > 0) {
				c(top.document.body).find('.layer-shade').hide()
			}
		...
		return e.run(), f.use('skin/layer.css'), f;
	}) : function() {
