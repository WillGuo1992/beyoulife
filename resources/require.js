...
// requirejs参数配置
require.config({
	baseUrl: "/beyoulife_new/resources",
	paths: {
		"domReady": "libs/domReady",
		"bootstrap": "libs/bootstrap/js/bootstrap.min",
		"bootstrap-table": "libs/bootstrap/bootstrap-table/bootstrap-table",
		"echarts": "libs/echarts",
		"jquery": "libs/jquery/jquery-2.1.0",
		"EasyDropdown": "libs/jquery.easydropdown",
		"jquery.pagination": "libs/jquery.pagination",
		"jquery.validate": "libs/jquery.validate",
		"qrcode": "libs/jquery.qrcode.min",
		"layer": "libs/layer/layer",
		"util": "modules/common/util",
		"WdatePicker": "libs/datepicker/WdatePicker",
		"Vue": "http://beyoulife-10039213.cos.myqcloud.com/libs/vue",
		"avalon": "libs/avalon",
		"Ajax": "libs/ajax",
...
(function(window) {
	...
	var siteName = "beyoulife";
	var domainName = "http://op.beyoulsc.com/";
	var qmServer = domainName + "beyoulife-bk/";
	var wxUrl = qmServer + '/wxbymenu/';
	var longRequest = 'ws://op.beyoulsc.com/beyoulife-bk/';
	var imgUrl = "http://beyoubj-op.oss-cn-beijing.aliyuncs.com/";
	Base.wxUrl = wxUrl;
	Base.wifi = domainName + "/beyoulife-wifi/";
	Base.domainName = domainName;
	Base.qmServer = qmServer;
	Base.longRequest = longRequest;
...
(function(window) {
	...
	Date.prototype.Format = function(fmt) { //author: meizz
		...
		/**
		 * @description 扩展string对象删除字符串中的html标签
		 */
	String.prototype.delHtmlTag = function() {
		return this.replace(/<[^>]+>/g, "").replace(/&nbsp;/ig, "");
	};
	/**
	 * @description 删除对象属性的前后空格
	 * @param {obj} object
	 */
	window['Trim'] = function(obj) {
			if(!obj) {
				return false;
			}
			for(var pro in obj) {
				if(obj.hasOwnProperty(pro)) {
					if(typeof obj[pro] === 'string') {
						obj[pro] = obj[pro].replace(/(^\s*)|(\s*$)/g, "");
					}
				}
			}
		}
		/**
		 * @description 屏蔽对象属性的前后空格
		 */
		//jquery 中扩展object 报错 修改为全局方法
		/*	Object.prototype.Trim = function() {
		 for(var pro in this) {
		 if(!this.hasOwnProperty(pro)) {
		 if(typeof this[pro] === 'string') {
		 this[pro] = this[pro].replace(/(^\s*)|(\s*$) /g, "");
		 }
		 }
		 }
		 }*/
